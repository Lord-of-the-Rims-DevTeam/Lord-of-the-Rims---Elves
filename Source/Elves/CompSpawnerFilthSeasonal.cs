...
namespace Elves
{
    public class CompSpawnerFilthSeasonal : ThingComp
    {
        ...
        private bool CanSpawnFilth
        {
            get
            {
                if (this.parent is Hive hive && !hive.CompDormant.Awake)
                {
                    return false;
                }
        ...
        public void TrySpawnFilth()
        {
            ...
            if (!CellFinder.TryFindRandomReachableCellNear(this.parent.Position, this.parent.Map, this.Props.spawnRadius, TraverseParms.For(TraverseMode.NoPassClosedDoors, Danger.Deadly, false), (IntVec3 x) => x.Standable(this.parent.Map), (Region x) => true, out IntVec3 c, 999999))
            {
                return;
            }
            FilthMaker.TryMakeFilth(c, this.parent.Map, this.Props.filthDef, 1);
        }
